...
<div class="wpim wpim-<?php echo esc_attr( $collection->getId() ) ?>">
	...
	foreach ( $markers as $marker ) {
		...
		$attrs = array(
			...
			'id="' . $marker->get_id() . '"',
			'class="wpim-marker wpim-marker--' . $marker->infowindow->get_event() . ' wpim--' . $marker->get_type() . ' wpim-' . $collection->getId() . '__' . $marker->get_id() . '"',
		);
		...
		if ( $marker->infowindow->get_content() ) {
			...
			echo '<div class="' . implode( ' ', $infowindowClass ) . '"><a class="wpim-infowindow__close" href="#"><i class="fa fa-times"></i></a><div class="wpim-infowindow__inner">';
